...
2. **`nodes/YealinkYmcs/YealinkYmcs.node.json`** — `"nodeVersion"` field (must match `package.json` version)
...
- Immediately update `YealinkYmcs.node.json` `nodeVersion` to match
